    ...
    pub fn update(&mut self, index: usize, text: &str) {
        match self.role {
            PositionRole::Start => {
                ...
                let overflow: usize = max(0, (index as isize) - (text.len() as isize) + 1)
                                      .try_into()
                                      .unwrap();

                let text_to_index: String = text
                                            .chars()
                                            .take(index+1)
                                            .collect();
                let lines_to_index: Vec<&str> = text_to_index
                                                .split("\n")
                                                .collect();

                self.line_num = max(0, (lines_to_index.len() as isize)-1)
                                .try_into()
                                .unwrap();

                let last_line: &str = lines_to_index
                                      .last()
                                      .unwrap()
                                      .to_owned();

                let col: usize = max(0, (last_line.len() as isize)-1)
                                 .try_into()
                                 .unwrap();
