...
mod message {
    use super::*;
    use crate::colors::{colored, Color};
    use chrono::Local;

    fn get_timestamp() -> String {
        return Local::now()
            .format(&colored(Color::Grey, "%H:%M:%S"))
            .to_string();
    }
    ...
    fn get_file(pos: &Position) -> String {
        colored(
            Color::LightYellow,
            &format!("\"{}:{}:{}\"", pos.file_name, pos.ln, pos.col),
        )
    }
    ...
    fn get_prev_line(pos: &Position) -> String {
        ...
        match prev_line {
            Some(x) => {
                prev_line_str = colored(Color::Grey, &format!("\n {} | \x1b[0m{}", pos.ln, x));
            }
            None => {
                prev_line_str = String::from("");
            }
        }
    ...
    fn get_line(start_pos: &Position, end_pos: &Position, color: Color) -> String {
        ...
        return colored(
            Color::Grey,
            &colored(Color::Grey, &format!(" {} | ", start_pos.ln + 1)),
        ) + &line[..start_pos.col]
            + &colored(color, &line[start_pos.col..end_pos.col])
            + &line[end_pos.col..];
    }
    ...
    fn generate_message_with_traceback(
        message_type: String,
        color: Color,
        name: &String,
        details: &String,
        ...
        format!(
            "{}\nFile {}{}\n{}\n{}",
            get_message(message_type, color, name, details),
            file,
            prev_line,
            line,
            underline
    ...
    pub fn get_message_with_traceback(
        message_type: String,
        color: Color,
        name: &String,
        details: &String,
        ...
        return generate_message_with_traceback(
            message_type,
            color,
            &name,
            &details,
...
impl Error {
    pub fn to_string(&mut self) -> String {
        message::get_message_with_traceback(
            String::from("ERROR"),
            Color::Red,
            &self.name,
            &self.details,
